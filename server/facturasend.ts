...
async function obtenerCatalogos() {
  ...
  try {
    const [deptResponse, distResponse] = await Promise.all([
      fetch(`https://api.facturasend.com.py/api/${FACTURASEND_CONFIG.tenantId}/departamentos`, {
        headers: { 'Authorization': `Bearer api_key_${FACTURASEND_CONFIG.apiKey}` }
      }),
      fetch(`https://api.facturasend.com.py/api/${FACTURASEND_CONFIG.tenantId}/distritos`, {
        headers: { 'Authorization': `Bearer api_key_${FACTURASEND_CONFIG.apiKey}` }
      })
    ]);
...
export async function verificarConexionFacturaSend(): Promise<{ disponible: boolean; mensaje: string }> {
  try {
    ...
    const response = await fetch(`https://api.facturasend.com.py/api/${FACTURASEND_CONFIG.tenantId}/test`, {
      method: 'GET',
      headers: {
...
export async function enviarFacturaFacturaSend(documento: FacturaSendDocumento): Promise<FacturaSendResponse> {
  try {
    ...
    const tenantId = FACTURASEND_CONFIG.tenantId;

    if (!apiKey) {
      console.error('❌ API Key de FacturaSend NO configurada');
      return {
        success: false,
        error: 'API Key de FacturaSend no configurada. Agregá FACTURASEND_API_KEY en Secrets.',
    ...
    const url = `https://api.facturasend.com.py/api/${tenantId}/lote/create?xml=true&qr=true`.replace("api.facturasend.com.py/api/", "api.facturasend.com.py/");
    console.log('🚀 URL FINAL ENVIADA (LIMPIEZA AGRESIVA):', url);
